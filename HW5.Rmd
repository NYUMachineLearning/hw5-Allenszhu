...
A look at the boosting test error graph shows that the graph is a little scattered. Before 1000 trees the points are kind of scattered, with the minimum MSE point landing at around 200 trees. The first visible dip in the line however, occurs at around 2100 trees. In order to optimize this model, it would be therefore recommended to run somewhere between those two points. The second lowest point on the plot occurs at around 1000 trees, and thus might be a good starting place to determine just how many trees need to be run to optimize the model.
